...
//добавляем размеры чтобы масштабировать текстуру 
float cubeSize = 6.0;
...
//Ambient Occlusion самозатенение точек поверхности
//в зависимости от положения других объектов
...
vec3 getLight(vec3 p, vec3 rd, float id) {
   ...
   //самозатенение
    ...
    //свет, отраженный от освещенных объектов
    //который освещает объекты рядом
